...
def xmlrpc_iface(request):  # noqa: ARG001
...
def proxy(xmlrpc_iface):
    ...
    return xmlrpc.client.ServerProxy(
        f'http://test:test@localhost:{port}/xmlrpc')


def test_authentication(xmlrpc_iface):
    port = xmlrpc_iface.server.server_address[1]
    proxy = xmlrpc.client.ServerProxy(f'http://localhost:{port}/xmlrpc')
    pytest.raises(xmlrpc.client.ProtocolError, proxy.GetVersion)
    proxy = xmlrpc.client.ServerProxy(
        f'http://wrong:creds@localhost:{port}/xmlrpc')
    pytest.raises(xmlrpc.client.ProtocolError, proxy.GetVersion)
    proxy = xmlrpc.client.ServerProxy(
        f'http://guest:guest@localhost:{port}/xmlrpc')
...
def test_simple_queries(proxy):
    ...
    assert set(proxy.GetServices()) == {'svc.inst', 'svc'}
    pytest.raises(xmlrpc.client.Fault, proxy.NonexistingMethod)
